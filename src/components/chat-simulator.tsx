...
export function ChatSimulator({ restaurants: businesses }: { restaurants: Business[] }) {
    ...
    return (
        <Card className="w-full h-[700px] flex flex-col shadow-2xl border-none ring-1 ring-white/10 dark:ring-white/5 bg-white/40 dark:bg-black/40 backdrop-blur-3xl rounded-[2.5rem] overflow-hidden neural-border">
            <CardHeader className="py-7 px-10 border-b border-white/10 dark:border-white/5 relative z-10 glass">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <CardTitle className="text-2xl font-black tracking-tighter text-gradient">
                                Chat Preview
                            </CardTitle>
                            <div className="flex items-center gap-2 mt-0.5">
                                <span className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Online</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <AnimatePresence initial={false}>
                            {messages.length === 0 && (
                                ...
                                >
                                    ...
                                    <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-3 tracking-tight">Start a conversation</h3>
                                    <p className="max-w-xs mx-auto text-muted-foreground font-medium leading-relaxed opacity-70">
                                        Select a business above and type a message to test your chatbot.
                                    </p>
                                </motion.div>
                            )}
                            ...
                            {isLoading && (
                                ...
                                >
                                    ...
                                    <div className="glass rounded-[2rem] rounded-tl-none py-5 px-6 shadow-2xl flex items-center gap-3">
                                        ...
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Typing...</span>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
            ...
            <CardFooter className="p-8 glass border-t border-white/10 relative z-20">
                ...
                >
                    <Input
                        ...
                        disabled={isLoading}
                        className="flex-1 h-14 px-6 rounded-[1.75rem] border-white/10 dark:border-white/5 focus-visible:ring-primary/20 shadow-inner bg-black/[0.02] dark:bg-white/[0.02] font-medium text-base transition-all"
                    />
                    <Button
                        type="submit"
                        size="icon"
